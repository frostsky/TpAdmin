...
if(@$_GET['c']=='success'){
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        ...
        // 设置字符集
        $link->query("SET NAMES '{$data['DB_CHARSET']}'");
        $link->server_info>=5.5 or die("<script>alert('请将您的mysql升级到5.5以上');history.go(-1)</script>");
        // 创建数据库并选中
        ...
        // 导入sql数据并创建表
        $admin_str=file_get_contents('./install.sql');
        $sql_array=preg_split("/;[\r\n]+/", str_replace('pre_',$data['DB_PREFIX'],$admin_str));
...
//*************************************数据库设置*************************************
    ...
    'DB_CHARSET'            =>  '{$data['DB_CHARSET']}',  // 数据库编码默认采用utf8
